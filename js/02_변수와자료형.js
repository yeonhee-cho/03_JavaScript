...
// 한 번 지정하면 변경 불가
// number3 = 50;
console.log("number3의 값 : " + number3);

// var의 경우에는 함수(=행동, 기능) 내부에 작성하면
// {} 블록의 개념 관꼐없이 어디서든 변수이름 호출 가능

function var확인기능() {
  if (true) {
    // 만약에 사실이라면 중괄호 내부에 존재하는 일들을 실행할 것이다.
    var abc = 10; // 보통 이런식으로 선언할 경우에는 if 중괄호 내부에서만 abc를 사용하고자 하는 의도가 큼
  }
  abc = 20;
  /* 
    abc는 {}를 탈출했기 때문에 사용이 불가능해야 하는게 맞지만 
...
function const확인기능() {
  if (true) {
    ...
    // ghi = 40부터는 문제가 발생하므로 let xyz = 10; 코드 실행 자체가 불가
